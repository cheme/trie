...
impl<'a, L: TrieLayout> StackEntry<'a, L> {
	fn new(node_data: &'a [u8], prefix: LeftNibbleSlice<'a>, is_inline: bool, mut meta: L::Meta)
		   -> Result<Self, Error<TrieHash<L>, CError<L>>>
	{
		let node = L::Codec::decode(node_data, &mut meta)
	...
	fn encode_node(mut self) -> Result<Vec<u8>, Error<TrieHash<L>, CError<L>>> {
		...
		Ok(match self.node {
			...
			Node::Leaf(partial, _) => {
				L::Codec::leaf_node(partial.right(), self.value, &mut self.meta)
			}
	...
	fn advance_child_index<I>(
		...
		proof_iter: &mut I,
		inline_meta: impl Fn() -> L::Meta,
	) -> Result<Self, Error<TrieHash<L>, CError<L>>>
		where
			I: Iterator<Item=(&'a [u8], L::Meta)>,
	{
		match self.node {
