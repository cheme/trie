...
pub enum Value<L: TrieLayout> {
	...
	Inline(Bytes),
	/// Hash of value bytes and size when accessible.
	Node(TrieHash<L>, Option<usize>),
...
impl<L: TrieLayout> Value<L> {
		...
		) -> ChildReference<TrieHash<L>>,
	{
		...
		let value = match &*self {
			...
			Value::Node(hash, size) => EncodedValue::Node(hash.as_ref(), *size),
			Value::NewNode(Some(hash), value) =>
				EncodedValue::Node(hash.as_ref(), Some(value.len())),
