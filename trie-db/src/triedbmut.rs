...
use crate::{
	...
	Bytes, CError, CachedValue, DBValue, ParentCountFor, Result, TrieAccess, TrieCache, TrieError,
...
pub struct TrieDBMutBuilder<'db, L: TrieLayout> {
	...
	cache: Option<&'db mut dyn TrieCache<L::Codec, L::CacheConf>>,
...
impl<'db, L: TrieLayout> TrieDBMutBuilder<'db, L> {
	...
	pub fn with_cache(mut self, cache: &'db mut dyn TrieCache<L::Codec, L::CacheConf>) -> Self {
	...
	L: TrieLayout,
{
	...
	cache: Option<&'a mut dyn TrieCache<L::Codec, L::CacheConf>>,
	...
	L: TrieLayout,
{
	...
	fn insert_inspector(
		&mut self,
		node: Node<L>,
		node_count: Option<ParentCountFor<L>>,
		key: &mut NibbleFullKey,
		value: Bytes,
		old_val: &mut Option<Value<L>>,
		...
		Ok(match node {
			...
			Node::Branch(mut children, stored_value) => {
				...
				if partial.is_empty() {
					...
					if let Some(child) = children[idx].take() {
						...
						let (new_child, changed) = self.insert_at(
							child,
							node_count.map(|p| (p, idx)),
							key,
							value,
							old_val,
						)?;
			...
			Node::Leaf(encoded, stored_value) => {
					...
					(!L::USE_EXTENSION && common < existing_key.len())
				{
					...
					key.advance(common);
					let augmented_low =
						self.insert_inspector(low, None, key, value, old_val)?.unwrap_node();
	...
	fn remove_inspector(
		...
		Ok(match (node, partial.is_empty()) {
			...
			(Node::Branch(children, val), true) => {
				...
				Action::Replace(self.fix(Node::Branch(children, None), node_count, *key)?)
			},
			(Node::NibbledBranch(n, children, val), true) => {
				self.replace_old_value(old_val, val, key.left());
				// always replace since we took the value out.
				Action::Replace(self.fix(
					Node::NibbledBranch(n, children, None),
					node_count,
					*key,
				)?)
			},
			...
			(Node::NibbledBranch(encoded, mut children, value), false) => {
				...
				if common == existing_length && common == partial.len() {
					...
					if let Some(value) = value {
						...
						let f = self.fix(
							Node::NibbledBranch(encoded, children, None),
							node_count,
							*key,
						);
						Action::Replace(f?)
				...
				if common == existing_length {
					...
					match self.remove_at(
						...
						Some((new_child, changed)) => {
							...
							match changed {
								true => Action::Replace(self.fix(
									Node::Extension(encoded, new_child.into()),
									node_count,
									prefix,
								)?),
								false =>
	...
	fn fix(
		...
		self.fix_inner(node, node_count, key, false)
	}
	fn fix_inner(
		&mut self,
		node: Node<L>,
			...
			Node::Branch(mut children, value) => {
				...
				match (used_index, value) {
					...
					(UsedIndex::One(a), None) => {
						...
						self.fix(new_node, node_count, key)
					},
			...
			Node::Extension(partial, child) => {
				...
				match child_node {
					Node::Extension(sub_partial, sub_child) => {
						...
						self.fix_inner(Node::Extension(partial, sub_child), count, key.into(), true)
					},
					Node::Leaf(sub_partial, value) => {
						// combine with node below.
						if let Some(hash) = maybe_hash {
	...
	L: TrieLayout,
{
	...
	fn insert(
		...
		let (new_handle, _changed) =
			self.insert_at(root_handle, None, &mut NibbleSlice::new(key), value, &mut old_val)?;
	...
	fn remove(&mut self, key: &[u8]) -> Result<Option<Value<L>>, TrieHash<L>, CError<L>> {
		...
		match self.remove_at(root_handle, None, &mut key_slice, &mut old_val)? {
