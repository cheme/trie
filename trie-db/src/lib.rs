...
pub trait Query<L: TrieLayout> {
	...
	fn record(&mut self, _hash: &TrieHash<L>, _data: &[u8], _depth: u32, _meta: &L::Meta) {}
}

impl<'a, L: TrieLayout> Query<L> for &'a mut Recorder<TrieHash<L>, L::Meta> {
	type Item = DBValue;
	fn decode(self, value: &[u8]) -> DBValue { value.to_vec() }
	fn record(&mut self, hash: &TrieHash<L>, data: &[u8], depth: u32, meta: &L::Meta) {
		(&mut **self).record(hash, data, depth, meta);
	}
}

impl<F, T, L: TrieLayout> Query<L> for F where F: for<'a> FnOnce(&'a [u8]) -> T {
	type Item = T;
	fn decode(self, value: &[u8]) -> T { (self)(value) }
}

impl<'a, F, T, L: TrieLayout> Query<L> for (&'a mut Recorder<TrieHash<L>, L::Meta>, F) where F: FnOnce(&[u8]) -> T {
	type Item = T;
...
pub trait TrieLayout: Default + Clone {
	...
	type Codec: NodeCodec<HashOut=<Self::Hash as Hasher>::Out, Meta = Self::Meta>;
	/// Additional data attached to node.
	type Meta: Meta;

	/// Default meta with state initialized from this layout.
	fn new_meta(&self) -> Self::Meta {
...
/// Meta info use for alternate hashing of nodes.
///
/// When use in proof an additional header
/// indicates a hash is written
...
pub trait Meta: Clone + MaybeDebug {
	/// If active is false, meta callback
	/// can be skipped, and meta initiated
	/// with default values.
	const ACTIVE: bool;
	...
	fn resolve_alt_hashing<C: NodeCodec>(&self) -> hash_db::AltHashing;
}

impl Meta for AltHashing {
