...
use trie_db::{Trie, TrieMut, DBValue, TrieDBBrokenIterator};
...
pub fn fuzz_that_cmp_impl(input: &[u8]) {
	...
	let mut error = 0;
	{
		use trie_db::TrieIterator;
			...
			let base_iter = trie.iter().unwrap();
			let mut iter = TrieDBBrokenIterator::new(&trie).unwrap();
			let prefix = &b"012"[..];
/*			if let Ok(_) = iter.seek(prefix) {
	...
	let (root2, tt, err15) = reference_trie2::mut_insert(&data);

	assert_eq!(&root[..], &root2[..]);
	assert_eq!(iter_res, tt);
	assert_eq!(error, err15);
...
fn test_it() {
	println!("{:x?}", &b"012"[..]);
	let data: Vec<u8> = vec![0x0,0x0,0x0,0xff,0xff,0xff,0xe2,0xe2,0xa,0xa,0x0,0x0,0xff,0x0,0x0,0xff,0xff,0xff,0xe2,0xe2,0xff,0xff,0xff,0xff,0xff,0xff,0xa,0xa,0x0,0x21,0x28,0xff,0x30,0x31,0x32,0x40,0x0,0x0,0x0,0x0,0x0,0x0,0x83,0x0,0x0,0x0,0x0,0x0,0x83,0x0,0x0,0xe2,0xff,0xff,0xff,0xff,0xff,0xff,0xa,0xa,0x0,0x21,0x28,0xff,0x30,0x31,0x32,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x5,0x0,0x0,0x0,0x0,0xe2,0x0,0x0,0x0,0x0,];
