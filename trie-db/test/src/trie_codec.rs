...
fn test_encode_compact<L: TrieLayout<N>, const N: usize>(
	...
	let mut partial_db = MemoryDB::<L, N>::default();
...
fn test_decode_compact<L: TrieLayout<N>, const N: usize>(
	encoded: &[Vec<u8>],
	...
	let mut db = MemoryDB::<L, N>::default();
	let (root, used) = decode_compact::<L, _, N>(&mut db, encoded).unwrap();
...
fn trie_compact_encoding_works_internal<T: TrieLayout<N>, const N: usize>() {
	let (root, mut encoded, items) = test_encode_compact::<T, N>(
		vec![
	...
	test_decode_compact::<T, N>(&encoded, items, root, encoded.len() - 1);
}
...
fn trie_decoding_fails_with_incomplete_database_internal<T: TrieLayout<N>, const N: usize>() {
	let (_, encoded, _) =
		test_encode_compact::<T, N>(vec![(b"alfa", &[0; 32]), (b"bravo", b"bravo")], vec![b"alfa"]);

	assert!(encoded.len() > 1);

	// Reconstruct the partial DB from the compact encoding.
	let mut db = MemoryDB::<T, N>::default();
	match decode_compact::<T, _, N>(&mut db, &encoded[..encoded.len() - 1]) {
		Err(err) => match *err {
...
fn encoding_node_owned_and_decoding_node_works() {
	...
	// Populate DB with full trie from entries.
	let mut recorder = {
		let mut db = <MemoryDB<ExtensionLayout, N>>::default();
		let mut root = Default::default();
		let mut recorder = Recorder::<ExtensionLayout, N>::new();
		{
			let mut trie = <TrieDBMutBuilder<ExtensionLayout, N>>::new(&mut db, &mut root).build();
			for (key, value) in entries.iter() {
				trie.insert(key, value).unwrap();
			}
		}

		let trie = TrieDBBuilder::<ExtensionLayout, N>::new(&db, &root)
	...
	for record in recorder.drain() {
		let node =
			<<ExtensionLayout as TrieLayout<N>>::Codec as NodeCodec<N>>::decode(&record.data)
				.unwrap();
		let node_owned = node.to_owned_node::<ExtensionLayout>().unwrap();

		assert_eq!(
			record.data,
			node_owned.to_encoded::<<ExtensionLayout as TrieLayout<N>>::Codec>()
		);
	}
}
