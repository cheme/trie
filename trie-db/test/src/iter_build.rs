...
fn compare_implementations_prefixed(data: Vec<(Vec<u8>, Vec<u8>)>) {
	compare_implementations_prefixed_internal::<HashedValueNoExtThreshold<1, CacheCounter>>(
		data.clone(),
	);
...
fn compare_implementations_h(data: Vec<(Vec<u8>, Vec<u8>)>) {
	compare_implementations_h_internal::<HashedValueNoExtThreshold<1, CacheCounter>>(data.clone());
