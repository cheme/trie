...
type MemoryDB<T, const N: usize> = memory_db::MemoryDB<
	...
	memory_db::HashKey<<T as TrieLayout<N>>::Hash>,
	DBValue,
>;
...
fn test_generate_proof<L: TrieLayout<N>, const N: usize>(
	entries: Vec<(&'static [u8], &'static [u8])>,
	keys: Vec<&'static [u8]>,
) -> (<L::Hash as Hasher>::Out, Vec<Vec<u8>>, Vec<(&'static [u8], Option<DBValue>)>) {
	// Populate DB with full trie from entries.
	let (db, root) = {
		let mut db = <MemoryDB<L, N>>::default();
		let mut root = Default::default();
		{
			let mut trie = <TrieDBMutBuilder<L, N>>::new(&mut db, &mut root).build();
	...
	let proof = generate_proof::<_, L, _, _, N>(&db, &root, keys.iter()).unwrap();
	let trie = <TrieDBBuilder<L, N>>::new(&db, &root).build();
...
fn trie_proof_works_internal2<T: TrieLayout<N>, const N: usize>() {
	let (root, proof, items) = test_generate_proof::<T, N>(
		vec![
			// "do" is at a hash-referenced branch node.
			(&b"do"[..], b"verb"),
			// "dog" is at a hash-referenced branch node.
	...
	verify_proof::<T, _, _, _, N>(&root, &proof, items.iter()).unwrap();
}

test_layouts!(trie_proof_works, trie_proof_works_internal);
fn trie_proof_works_internal<T: TrieLayout<N>, const N: usize>() {
	let (root, proof, items) = test_generate_proof::<T, N>(
		test_entries(),
		vec![
...
fn trie_proof_works_for_empty_trie_internal<T: TrieLayout<N>, const N: usize>() {
	let (root, proof, items) =
		test_generate_proof::<T, N>(vec![], vec![b"alpha", b"bravo", b"\x42\x42"]);

	verify_proof::<T, _, _, _, N>(&root, &proof, items.iter()).unwrap();
}

test_layouts!(test_verify_duplicate_keys, test_verify_duplicate_keys_internal);
fn test_verify_duplicate_keys_internal<T: TrieLayout<N>, const N: usize>() {
	let (root, proof, _) = test_generate_proof::<T, N>(test_entries(), vec![b"bravo"]);

	let items = vec![(b"bravo", Some(b"bravo")), (b"bravo", Some(b"bravo"))];
	assert!(if let Err(VerifyError::DuplicateKey(key)) =
		verify_proof::<T, _, _, _, N>(&root, &proof, items.iter(),)
	{
		key == b"bravo".to_vec()
	} else {
		false
...
fn test_verify_extraneaous_node_internal<T: TrieLayout<N>, const N: usize>() {
	let (root, proof, _) = test_generate_proof::<T, N>(test_entries(), vec![b"bravo", b"do"]);
...
fn test_verify_extraneaous_value_internal<T: TrieLayout<N>, const N: usize>() {
	let (root, proof, _) = test_generate_proof::<T, N>(test_entries(), vec![b"doge"]);
...
fn test_verify_extraneous_hash_reference() {
	const N: usize = 16;
	let (root, proof, _) = test_generate_proof::<NoExtensionLayout, N>(test_entries(), vec![b"do"]);

	let items = vec![(&b"alfa"[..], Some(&[0; 32][..])), (&b"do"[..], Some(&b"verb"[..]))];
	match verify_proof::<NoExtensionLayout, _, _, _, N>(&root, &proof, items.iter()) {
...
fn test_verify_invalid_child_reference_internal<T: TrieLayout<N>, const N: usize>() {
	let (root, proof, _) = test_generate_proof::<T, N>(test_entries(), vec![b"bravo"]);
	...
	match verify_proof::<T, _, _, _, N>(&root, &proof, items.iter()) {
...
fn test_verify_value_mismatch_some_to_none_internal<T: TrieLayout<N>, const N: usize>() {
	let (root, proof, _) = test_generate_proof::<T, N>(test_entries(), vec![b"horse"]);
...
fn test_verify_value_mismatch_none_to_some_internal<T: TrieLayout<N>, const N: usize>() {
	let (root, proof, _) = test_generate_proof::<T, N>(test_entries(), vec![b"alfa", b"bravo"]);
...
fn test_verify_incomplete_proof_internal<T: TrieLayout<N>, const N: usize>() {
	let (root, mut proof, items) = test_generate_proof::<T, N>(test_entries(), vec![b"alfa"]);
...
fn test_verify_root_mismatch_internal<T: TrieLayout<N>, const N: usize>() {
	let (root, proof, _) = test_generate_proof::<T, N>(test_entries(), vec![b"bravo"]);
...
fn test_verify_decode_error_internal<T: TrieLayout<N>, const N: usize>() {
	let (root, mut proof, items) = test_generate_proof::<T, N>(test_entries(), vec![b"bravo"]);
