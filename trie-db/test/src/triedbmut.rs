...
use reference_trie::{
	...
	HashedValueNoExt, HashedValueNoExtThreshold, HashedValueNoExtThresholdWithSize,
	NoExtensionLayout, RefHasher, ReferenceNodeCodec, ReferenceNodeCodecNoExt, TestTrieCache,
};
use trie_db::{
...
fn register_proof_without_size() {
	...
	//
	// TODO factor with previous test
	use hash_db::{AsHashDB, Prefix};
	use reference_trie::HashedValueNoExtThresholdWithSize;
	use std::{cell::RefCell, collections::HashMap};
	use trie_db::TrieDB;

	type Layout = HashedValueNoExtThresholdWithSize;
	type MemoryDB = memory_db::MemoryDB<RefHasher, PrefixedKey<RefHasher>, DBValue>;
	let x = [
		(b"test1".to_vec(), vec![1; 33]),
		(b"test1234".to_vec(), vec![2; 36]),
		(b"te".to_vec(), vec![3; 33]),
	];
	...
	let mut memdb = ProofRecorder { db: memdb.clone(), record: Default::default() };

	let root_proof = root.clone();
	{
		let trie = TrieDBMut::<Layout>::from_existing(&mut memdb, &mut root);
		// touch te size, not value.
		trie.get_size(b"te").unwrap();
	...
	let memdb_from_proof = db_unpacked.clone();
	let root_proof = root_unpacked.clone();
	{
		use trie_db::Trie;
		let trie = TrieDB::<Layout>::new(&memdb_from_proof, &root_proof);
		assert!(trie.get_size(b"te").unwrap().is_some());
		assert!(matches!(trie.get(b"te").map_err(|e| *e), Err(TrieError::IncompleteDatabase(..))));
	}
}
